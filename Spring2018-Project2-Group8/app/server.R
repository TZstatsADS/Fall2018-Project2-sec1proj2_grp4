...
shinyServer(function(input, output){
  ...
  output$plot11 <- renderPlot({
    ...
    iips_data %>%
      ...
      geom_bar(stat="identity", fill="turquoise", color="blue") +
  ...
  output$plot12 <- renderPlot({
    ...
    ggplot(hospital.ratings.df) +
      geom_bar(aes(x=reorder(State, -HospitalRating), y=HospitalRating), stat="identity", color="white", fill="blue") +
  ...
  output$read1<- renderText({"This app was developed by a team of 5 dedicated members in Fall 2018:"})
  output$read2<- renderText({"Ghada, Jerfel  email: l: gj2261@barnard.edu"})
  output$read3<- renderText({"Jin, Peiqi     email: l: pj2324@columbia.edu"})
  output$read4<- renderText({"li, Xiaoyi     email: l: xl2694@columbia.edu"})
  output$read5<- renderText({"Wang, Zehan    email: l: zw2457@columbia.edu"})
  output$read6<- renderText({"Wei, Xiaojie   email: l: xw2536@columbia.edu"})
