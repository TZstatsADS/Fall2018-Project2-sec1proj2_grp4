...
shinyServer(function(input, output){
  ...
  output$plot11 <- renderPlot({
    ...
    iips_data %>%
      ...
      arrange(-m.ratio) %>%
      #tail(25) %>%
      ggplot(aes(x=reorder(Provider.State, m.ratio), y=m.ratio)) +
      ...
      geom_text(aes(x=Provider.State, y=round(m.ratio,2), label=round(m.ratio,2), pos=2, vjust=-0.5),angle=45) +
      scale_y_continuous(limits = c(0,0.40), labels = scales::percent)
